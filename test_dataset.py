...
class ClayDatasetGoalMeasure(torch.utils.data.Dataset):
    def __init__(self, dataset_dir, pred_horizon, n_datapoints, n_raw_trajectories, center_action):
        ...
        :param center_action: whether to center the action before normalizing
        """
        super(ClayDatasetGoalMeasure).__init__()
        self.dataset_dir = dataset_dir
        self.pred_horizon = pred_horizon
        self.n_datapoints = n_datapoints
        self.n_raw_trajectories = n_raw_trajectories
    ...
    def __getitem__(self, idx):
        ...
        # parse self.dataset_dir + '/Trajectory' + str(raw_traj_idx) + '/demo_params.txt'
        # line 1: Initial Cylinder Height: X [cm]
        # line 2: Final Bowl Diameter: Y [cm]
        # we want to get the number Y in line 2
        with open(traj_path + '/demo_params.txt', 'r') as f:
            lines = f.readlines()
            final_bowl_diameter = float(lines[2].split(':')[1].strip().replace('[cm]', '')) / 100.0
        
        goal = np.array([final_bowl_diameter])
