...
def sculptdiff_generate_actions(pointbert, projection_head, goal_head, noise_scheduler, noise_pred_net, pointcloud, numpy_goal, nagent_pos, obs_horizon, action_dim, num_diffusion_iters, device):
    B = 1
...
def experiment_loop(fa, cam1, cam2, cam3, cam4, cam5, pcl_vis, save_path, goal_str, ckpt_dir, done_queue, centered_action, pred_horizon, execute_horizon, numpy_goal, collision_check):
    ...
    in_progress = True
    while in_progress:
        naction, total_time = sculptdiff_generate_actions(pointbert, projection_head, goal_head, noise_scheduler, noise_pred_net, pointcloud, numpy_goal, nagent_pos, obs_horizon, action_dim, num_diffusion_iters, device)
        ...
        for j in range(execute_horizon):
            ...
            if iter > 6 and collision_check:
                ...
                while collision and n_checks < 10:
                    ...
                    naction, total_time = sculptdiff_generate_actions(pointbert, projection_head, goal_head, noise_scheduler, noise_pred_net, og_pointcloud, numpy_goal, og_nagent_pos, obs_horizon, action_dim, num_diffusion_iters, device)
                    pred_action = naction[0]
                    termination_pred = pred_action[:,7]
                    action_pred = (pred_action[:,0:7] + 1.0) / 2.0
                    action_pred = action_pred * (a_maxs7d - a_mins7d) + a_mins7d
    ...
    results_dict = {'n_actions': n_action, 'avg planning time': np.mean(planning_time_list)}
